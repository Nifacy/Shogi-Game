...
class StateConverter(GameState):
    @classmethod
...
def encode_state(state: GameState) -> str:
    return json.dumps(StateConverter.encode(state))


def decode_state(encoded: Union[str, bytes]) -> GameState:
    return StateConverter.decode(json.loads(encoded))
